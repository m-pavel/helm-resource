...
PROJECT_GH="m-pavel/$PROJECT_NAME"
...
: "${HELM_PLUGIN_DIR:="$HELM_HOME/plugins/helm-resource"}"
...
rmTempDir() {
  if [ -d "${HELM_TMP:-/tmp/helm-resource-tmp}" ]; then
    rm -rf "${HELM_TMP:-/tmp/helm-resource-tmp}"
  fi
}
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/resource/bin/resource"
  if [ "${OS}" = "windows" ]; then
...
exit_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/m-pavel/helm-resource.\n'
  fi
  exit $result
}
